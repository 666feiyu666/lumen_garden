...
当前版本已进入 **Phase 2 第一章 demo**：首页入口为 `教程 / 荧光森林 / 荧光花园 / 设置`。主线已扩展为五房间线性森林，包含背包、图鉴、普通种子、稳定图案、流光种机关和摇曳花枝按钮门；荧光花园也已从占位页扩展为六块可点击花圃的 hub。Phase 1 的花园谜题与种植花谱代码仍保留为规则资产和回归测试基础，但不再作为首页主入口。

## 当前版本

- `教程`：静态生命游戏规则页，使用 `assets/sprites/tutorial_menu.png` 作为底图，只展示基础生死规则和小棋盘示意。
- `荧光森林`：第一章剧情模式。进入后先显示 `Introduction_1.png` 开场漫画，再依次通过第 0-4 房间：移动教学、普通植物、稳定 `Block` 图鉴、`Glider` 补种机关、`Blinker` 按钮门。
- `荧光花园`：当前为六块可点击花圃的 hub，使用 `garden.png` 作为底图、`puzzle_board.png` 作为花圃。点击空花圃会根据背包和图鉴尝试种植。
- `设置`：使用 `setting.png` 弹窗。首页按 `Esc` 会打开设置，不会直接退出；关卡中按 `Esc` 也打开设置，并提供退出关卡选项。
- 第一章结尾：第 04 房间出口触发 `end_1.png` 结尾漫画，随后进入荧光花园 hub。
...
| 首页快捷键 | `G` / `T` / `0` 打开教程，`F` 进入荧光森林，`P` 打开荧光花园，`Esc` 打开设置 |
...
| 荧光森林背包/图鉴 | 点击右上角按钮查看；底部对话框最多显示 5 秒，点击对话框可收起 |
| 荧光森林关卡 | `R` 重试当前房间，`Esc` 打开设置 |
| 荧光花园 | 点击六块花圃进行种植；点击已种花圃查看内容；点击底部对话框可收起 |
| 设置 | 点击按钮调整窗口大小、语言、音量、静音、返回、退出关卡或退出游戏 |

## 验证

回归测试覆盖 Phase 1 规则与种植验证、Phase 2 首页入口、静态教程、剧情漫画、五房间森林流程、背包图鉴、六花圃荧光花园 hub、设置弹窗和无窗口 UI smoke tests：
